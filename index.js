...
const asana = axios.create({
  ...
  headers: { Authorization: `Bearer ${process.env.ASANA_TOKEN?.trim()}`, "Content-Type": "application/json" },
});
...
// Debug endpoint - borrar después
app.get("/debug-token", async (req, res) => {
  try {
    const token = process.env.ASANA_TOKEN;
    const result = await axios.get("https://app.asana.com/api/1.0/users/me", {
      headers: { Authorization: `Bearer ${token?.trim()}` }
    });
    res.json({ ok: true, user: result.data.data.name, tokenLength: token?.length });
  } catch (err) {
    res.json({ error: err.response?.data, tokenLength: process.env.ASANA_TOKEN?.length, tokenStart: process.env.ASANA_TOKEN?.substring(0,5) });
  }
});
